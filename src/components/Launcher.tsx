...
const Launcher: React.FC<{ onLogout?: () => void }> = ({ onLogout }) => {
  ...
  const isNoPremium = accountType === "nopremium";
  const isPremium = accountType === "premium";
  const restrictVersionsUntilBuild1 = isNoPremium || isPremium;

  const latestVersion =
    availableVersions.length > 0 ? availableVersions[0] : null;

  // If the user is in a login mode (Premium or No Premium), only allow selecting
  ...
  return (
    ...
    >
      ...
      <ConfirmModal
        ...
        onCancel={() => setHostServerWarningOpen(false)}
        onConfirm={() => {
          setHostServerWarningOpen(false);
