...
const computeDynamicAliases = () => {
  ...
  const dynamicRoot = path.join(__dirname, "dynamic_modules");
  const hasDynamicRoot = (() => {
    try {
...
export default defineConfig(({ mode }) => {
  ...
  const launcherSecretKey =
    env.LAUNCHER_SECRET_KEY ?? process.env.LAUNCHER_SECRET_KEY ?? "";
